# 서재린

<img src ="https://github.com/seojaelynn04-lab/resume-profile-test/blob/main/%ED%94%84%EB%A1%9C%ED%95%84.jpg" width="150" alt="도전을 두려워하지 않는 서재린입니다." align="left">

### 기본 학적 (Profile) 
**생년월일** : 2004. 01. 28  </br>
**전공** : 사이버보안학과 </br>
**E-mail** : seojaelynn04@google.com </br>
**Phone** : 010-2554-8704 </br>
**Git-hub** : [seojaelynn04-lab] https://github.com/seojaelynn04-lab </br>
  
<hr>

### 보유 기술 (Skills)  

<p align="left"> <!--뱃지 달기를 위해 ai 도움 및 무료 이미지 오픈소스 활용용--> 
<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python"/>
<img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=java&logoColor=white" alt="Java"/>
<img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white" alt="C++"/>
<img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" alt="HTML"/>
<img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" alt="Linux"/>
<img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" alt="MySQL"/>
</p>

#### Programming Languages

<table>
  <tr>
    <th>언어</th>
    <th>숙련도</th>
  </tr>
  
  <tr>
    <td>Python</td>
    <td>🔵🔵🔵🔵⚪️</td>
  </tr>
  
  <tr>
    <td>Java</td>
    <td>🔵🔵🔵⚪️⚪️</td>
  </tr>
  
  <tr>
    <td>C++</td>
    <td>🔵🔵🔵🔵⚪️</td>
  </tr>
</table>

#### Markup Languages 
<table>
  <tr>
    <th>언어</th>
    <th>숙련도</th>
  </tr>
  
  <tr>
    <td>HTML</td>
    <td>🔵🔵🔵🔵⚪️</td>
  </tr>
</table>

#### Operating Systems 
<table>
  <tr>
    <th>시스템</th>
    <th>숙련도</th>
  </tr>
  
  <tr>
    <td>Linux</td>
    <td>🔵🔵🔵🔵⚪️</td>
  </tr>
</table>

#### Database
<table>
  <tr>
    <th>시스템</th>
    <th>숙련도</th>
  </tr>
  
  <tr>
    <td>MySQL</td>
    <td>🔵🔵🔵⚪️⚪️</td>
  </tr>
</table>

<hr>

### 프로젝트 (Projects) 

<table>
  <tr>
    <th>프로젝트명</th>
    <th>주요 성과</th>
    <th>링크</th>
  </tr>
  
  <tr>
    <td>
      <strong>랜섬웨어 공격을 막는 Entropy&Malice_score 방어기법 구현</strong> <br>
      <small>(2025-2학기 / 3명 / <code>C++</code>)</small>
    </td>
    <td>
      * Entropy 계산 후 임계치 및 가중치 부가 <br>
      • Entropy 기반으로 Malice_score 계산 후 악성코드 판정 <br>
      * fuse 환경에서 랜섬웨어의 공격시 상황 판단하고 침입하지 못하게 가드 설치 <br>
      • 부분 암호화를 통한 파일 훼손 방지
    </td>
    <td align="center">
      <a href="https://github.com/system-security-0x/ransonware-battle/blob/main/test/test_result/blue2.c">GitHub</a> 
    </td>
  </tr>

  <tr>
    <td>
      <strong>API 활용한 문화생활 정보제공 웹 개발</strong> <br>
      <small>(2024-1학기 / 2명 / <code>HTML,CSS,JavaScript</code>)</small>
    </td>
    <td>
      * 네이버지도 API 활용한 지도 제공 <br>
      • KOBIS 영화관 API 활용한 주간/주말 영화 순위 및 영화 정보 제공 <br>
      * 공연, 도서, 영화 관련 정보 한 페이지에서 제공 <br>
      • 문화생활 정보를 바탕으로한 추천 기능 제공 
    </td>
    <td align="center">
      <a href="https://github.com/seojaelynn04-lab/HTML-Group_Project">GitHub</a> 
    </td>
  </tr>

  <tr>
    <td>
      <strong>Python 날씨 및 할 일 목록 개발</strong> <br>
      <small>(2023-2학기 / 5명 / <code>파이썬</code>)</small>
    </td>
    <td>
      * 사용자의 경로를 파악 및 해당 지역의 날씨 제공 <br>
      • 날짜별 할 일 목록 제공 및 체크리스트 제작 <br>
      * 도시 어디에서든 날씨 정보를 제공 <br>
      • 해야 할 업무에 대한 체계적 분석 기능 제공 
    </td>
    <td align="center">
      <a href="https://github.com/seojaelynn04-lab/-api-team_project/tree/main">GitHub</a>
    </td>
  </tr>
</table>

<hr>

### 대외활동 (Activities) 

<table>
  <tr>
    <th>기간</th>
    <th>활동 내역</th>
    <th>주요 역할 및 경험</th>
  </tr>
  
 <tr>
    <td>
      <strong>[2025.03 ~ 2026.02]</strong>
    </td>
    <td>
      <strong>[단국대학교] 사이버보안학과 학생회 (부회장)</strong>
    </td>
    <td>
      • 학생회 총괄 운영 및 대표 역할 수행 <br>
      • 신입생 오리엔테이션 총괄 기획 및 인솔
    </td>
  </tr>
  
  <tr>
    <td>
      <strong>[2024.03 ~ 2025.02]</strong>
    </td>
    <td>
      <strong>[단국대학교] 사이버보안학과 학생회 (사무부 부장)</strong>
    </td>
    <td>
      • 사무부 팀원(1명) 관리 및 업무 총괄 <br>
      • 학과 내 모든 축제 관련 예산안 계획 및 작성 <br>
      • 신입생 인솔
    </td>
  </tr>

  <tr>
    <td>
      <strong>[2023.03 ~ 2024.02]</strong>
    </td>
    <td>
      <strong>[단국대학교] 사이버보안학과 학생회 (기획운영부 부원)</strong>
    </td>
    <td>
      • 학과 내 OOTD 대결 행사 제작<br>
      • 학과 행사 참여 및 운영 보조
    </td>
  </tr>
</table>

<table>
  <tr>
    <th>기간</th>
    <th>기관/회사명</th>
    <th>담당 업무</th>
  </tr>
  
  <tr>
    <td>
      <strong>[2025.03 ~ 2025.08]</strong>
    </td>
    <td>
      <strong>교내 근로 장학생</strong>
    </td>
    <td>
      • [교수학습개발센터] 사무실 행정 보조 <br>
      • 학업 관련 문의 및 전화 응대
    </td>
  </tr>
  
  <tr>
    <td>
      <strong>[2024.08 ~ 2024.12]</strong>
    </td>
    <td>
      <strong>[보드게임카페 아르바이트]</strong>
    </td>
    <td>
      • 카페 바리스타 (파트타임)<br>
      • 고객 응대 및 매장 관리
    </td>
  </tr>
</table>

<hr>

### 자기소개 (Introduction) 
<p>사용자의 편의성을 가장 먼저 생각하는 웹 개발자 서재린입니다. 저에게 있어 편의성이란 단순히 보기 좋게 꾸미는 것이 아니라, <strong>"보이지 않는 설계의 견고함"</strong>을 의미합니다. 사이버보안학과를 전공하며 <em>보안이 강화된 시스템 구조</em>의 중요성을 배웠습니다. 이를 바탕으로 <code>Linux</code>환경과 <code>Python</code> 을 활용해 내부적으로도 안전하고 신뢰할 수 있는 API 를 제작하는 것을 기본으로 삼고 있습니다. </p>

<ul>
  <li>
    <details> <!--open 옵션으로 페이지를 열었을 때부터 내용이 나오도록 함--> 
    <summary><strong>성장하는 리더십과 책임감     <- 눌러보세요 </strong></summary> <br>
    대학생활 3년간 꾸준히 학과 학생회 활동을 통해 소통 능력과 솔선수범하는 태도를 배웠습니다. 1학년 때는 <strong>기획운영부원</strong>으로서 행사를 기획하고 보조하였으며, 2학년 때는 행사 예산을 총괄하는 <strong>사무부장</strong>으로 활동했습니다. 3학년에는 추천을 받아 학과 내 <strong>부회장</strong>으로 신입생 오리엔테이션 총괄 기획과 같은 다양한 프로젝트를 이끌었습니다. 3년간의 학생회 활동은 마치 매주 하나의 프로젝트를 수행하는 과정과 같았습니다. 다양한 의견들을 조율하고, 최적의 결정을 내리며, 예기치 못한 돌발 상황에 유연하게 대처하는 법을 배웠습니다. 이러한 경험은 수많은 사람과 소통하고 협력하며 일 해야하는 개발자에게는 큰 자산이 될 것입니다. <br>
    </details>
  </li>
  
 <br>
  
  <li>
    <details>
    <summary><strong>꾸준함에 대한 증명</strong></summary> <br> <!-- 눌러보세요-->
    저는 <strong>도전</strong>이라는 단어를 좋아합니다. 하지만 여러 도전을 선언해놓고 끝맺지 못한다면 그것은 온전한 도전이라 생각하지 않습니다. 저는 도전이 일회성이 되지 않도록 <strong>꾸준함</strong>을 더하려 노력했습니다. 매년 겨울방학마다 <strong>미국 단기 어학연수</strong>에 참여한 것이 그 증거입니다. 낯선 환경에 적응하고 익숙하지 않은 언어로 소통하는 것은 마치 복잡한 코드를 디버깅하는 것과 같았습니다. 실제 코드를 작동시키기 위해 오류를 끊임없이 찾아내야 하듯, 저의 꾸준함은 개발자로서 무기가 되었습니다. <br>
      </details>
  </li>
  
 <br>
 
  <li>
     <details>
    <summary><strong>기여할 수 있는 점</strong></summary> <br> <!-- 눌러보세요-->
   저의 강점은 명확합니다. 사이버보안 전공으로 다진 <strong>보안성</strong>, 학생회 활동으로 기른 <strong>책임감</strong> 그리고 어학연수로 증명한 <strong>꾸준함</strong>을 저의 기술 역량에 더하는 것입니다. 동료들과 적극적으로 소통하며 시너지를 내고, 궁극적으로는 "사용자가 편리하고 안심할 수 있는" 견고한 API와 웹 서비스를 만드는 개발자가 되고자합니다.
       </details>
  </li>
</ul>

<hr>


#### <em> "보이지 않는 곳에서 신뢰를 설계하는 웹 개발자, 서재린입니다." </em> 
